...
用法:
    ...
    python cli.py --profile compilable --cases compilable
    python cli.py --profile performance --cases performance
...
def run_single_case(case: dict, scenario: str, skill_content: str,
    ...
    if dry_run:
        ...
        baseline_llm = llm_judge.judge(input_code, baseline_output, reference_code, rubric)
        enhanced_llm = llm_judge.judge(input_code, enhanced_output, reference_code, rubric)
        print("完成")
...
def main():
    ...
    parser.add_argument("--cases", required=True,
                        help="测试场景: project_gen, compilable, performance, all")
    ...
    if args.profile == "all":
        profiles_to_run = list(profile_map.keys())
    else:
    ...
    for scenario in scenarios_to_run:
        ...
        skill_path = os.path.join(BASE_DIR, profile_map.get(scenario, ""))
        ...
        for i, case in enumerate(cases):
            ...
            result = run_single_case(case, scenario, skill_content,
                                    agent_runner, llm_judge,
                                    dry_run=args.dry_run)
            all_results.append(result)
            
            case_output_dir = os.path.join(output_dir, "cases", case["id"])
